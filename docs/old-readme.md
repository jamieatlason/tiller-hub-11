...
- `CF_ACCESS_CLIENT_ID` + `CF_ACCESS_CLIENT_SECRET` — so tiller-cli can authenticate back to the hub over the public internet
- `ANTHROPIC_API_KEY`

With CF Containers, the container is a DO on the same Worker — no CF Access credentials needed for tiller-cli to talk to the hub (internal communication). That drops 2 long-lived credential pairs from every container.
...
| `container/entrypoint.sh` | Drop rclone config/sync, SSH setup. Keep git auth, ttyd, tiller-cli. 106 → ~30 lines                                                                             |
...
1. `POST /api/envs` with a `repoUrl` → creates a container, clones the repo into `/workspace`, starts tiller-cli + ttyd
